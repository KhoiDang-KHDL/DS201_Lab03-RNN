import torch
from torch.nn import functional as F
from torch.utils.data import Dataset

import json
import os
import string

def collate_fn(items:dict) -> torch.Tensor:
    input_ids = [item['input_ids'] for item in items]
    max_len = max([input.shape[0] for input in input_ids])
    input_ids = [
        F.pad(
            input,
            pad=(0, max_len - input.shape[0]),
            mode='constant',
            value=0
        ).unsqueeze(0) for input in input_ids
    ]
    label_ids = [item['label'].unsqueeze(0) for item in items]
    return {
        'input_ids': torch.cat(input_ids, dim=0),
        'label': torch.cat(label_ids, dim=0)
    }

class Vocab:
    def __init__(self, path: str): # Đổi type hint thành str vì os.listdir nhận str
        all_words = set()
        labels = set()
        
        # Kiểm tra xem đường dẫn có tồn tại không
        if not os.path.exists(path):
            raise FileNotFoundError(f"Không tìm thấy đường dẫn: {path}")

        # Lặp qua các file trong thư mục
        for filename in os.listdir(path):
            full_filepath = os.path.join(path, filename)
            
            # QUAN TRỌNG: Chỉ xử lý nếu là file VÀ có đuôi .json
            if os.path.isfile(full_filepath) and filename.endswith('.json'):
                try:
                    # Thêm encoding='utf-8' để đọc tiếng Việt
                    with open(full_filepath, 'r', encoding='utf-8') as f:
                        data = json.load(f)
                        
                    for item in data:
                        sentence: str = item['sentence']
                        sentence = self.preprocess_sentence(sentence)
                        all_words.update(sentence.split())
                        labels.add(item['topic'])
                except Exception as e:
                    print(f"Bỏ qua file {filename} do lỗi: {e}")
                    continue
        
        self.bos = "<s>"
        self.pad = "<p>"
        
        self.w2i = {
            word: idx for idx, word in enumerate(all_words, start=2)
        }
        self.w2i[self.bos] = 1
        self.w2i[self.pad] = 0
        
        self.i2w = {idx: word for word, idx in self.w2i.items()}
        
        self.label2i = {label: idx for idx, label in enumerate(labels)}
        self.i2label = {idx: label for label, idx in self.label2i.items()}
        
    @property 
    def n_labels(self) -> int:
        return len(self.label2i)
    
    @property
    def len(self) -> int:
        return len(self.w2i)
    
    def preprocess_sentence(self, sentence: str) -> str:
        translator = str.maketrans('', '', string.punctuation)
        sentence = sentence.lower()
        sentence = sentence.translate(translator)
        return sentence 
    
    def encode_sentence(self, sentence: str) -> list[int]:
        sentence = self.preprocess_sentence(sentence)
        words = sentence.split()
        words = [self.bos] + words
        # Handle unknown words (optional safety)
        words_ids = [self.w2i[word] for word in words if word in self.w2i]
        
        return torch.tensor(words_ids).long()
    
    def encode_label(self, label: str) -> torch.Tensor:
        label_idx = self.label2i[label]
        return torch.tensor(label_idx) 
    
    def decode_label(self, label_idx: torch.Tensor) -> str:
        label_idx = label_idx.tolist()
        labels = [self.i2label[idx] for idx in label_idx]
        return labels

class UIT_VSFC(Dataset):
    def __init__ (self, path: str, vocab: Vocab):
        super().__init__()
        
        self._data = json.load(open(path, 'r', encoding='utf-8'))
        self.vocab = vocab
        
    def __len__(self) -> int:
        return len(self._data)
    
    def __getitem__(self, index: int) -> dict:
        item = self._data[index]
        sentence = item['sentence']
        label = item['topic']
        
        input_ids = self.vocab.encode_sentence(sentence)
        label_id = self.vocab.encode_label(label)
        
        return {
            'input_ids': input_ids,
            'label': label_id
        }